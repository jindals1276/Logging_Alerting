        ...
        try:
            ...
            with engine._lock:
                ...
                for bucket_key in engine._buckets:
                    self.assertLessEqual(bucket_key, datetime.now(timezone.utc))
        finally:
            engine.shutdown()
...
class TestWindowSliding(unittest.TestCase):
    ...
    def test_slide_evicts_old_buckets(self):
        ...
        with self.engine._lock:
            ...
            self.engine._window_start = datetime.now(timezone.utc) - timedelta(seconds=3)
    ...
    def test_slide_on_empty_buckets_is_safe(self):
        ...
        with self.engine._lock:
            self.engine._window_start = datetime.now(timezone.utc) - timedelta(seconds=15)
            # Should not raise.
            self.engine._slide_window()
